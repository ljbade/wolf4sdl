...
#define MAXVIEWWIDTH            800     // NOTYET
...
extern SDL_Surface *backgroundSurface;
...
//#define FixedMul(a,b) (((a)>>16)*((b)&0xffff0000)+(((a)&0xffff)*((b)>>16))+(((a)>>16)*((b)&0xffff))+((((a)&0xffff)*((b)&0xffff))>>16))
//#define FixedMul(a,b) (((a)>>8)*((b)>>8)>>8)
...
#define DEBUGMSG() { FILE *file = fopen("debug.txt", "a"); \
    if(file) { fprintf(file, "file: %s line: %d\n\r", __FILE__, __LINE__); fclose(file); }}
