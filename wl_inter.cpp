...
void EndSpear(void)
{
        ...
        PrintX = 0;
        PrintY = screenheight - 20;
        VWB_Bar(0,screenheight - 20,screenwidth,20,0);
...
#define TIMEY   8
    ...
    VWB_Bar(0,0,screenwidth,screenheight,VIEWCOLOR);
    ...
    VWB_Bar (vidbuf, backgroundPitch, 0,0,screenwidth,screenheight-STATUSLINES,VIEWCOLOR);
    ...
    VWB_DrawPic(vidbuf, backgroundPitch, 0,0,C_ENDRATIOSPIC);
...
#ifndef JAPDEMO
    VWB_DrawPic (vidbuf, backgroundPitch, 8,4,L_BJWINSPIC);
#endif
    ...
    i = TIMEX*8+1;
    VWB_DrawPic(vidbuf, backgroundPitch, i,TIMEY*8,L_NUM0PIC+(min/10));
    i += 2*8;
    VWB_DrawPic(vidbuf, backgroundPitch, i,TIMEY*8,L_NUM0PIC+(min%10));
    i += 2*8;
    Write(i/8,TIMEY,":");
    i += 1*8;
    VWB_DrawPic(vidbuf, backgroundPitch, i,TIMEY*8,L_NUM0PIC+(sec/10));
    i += 2*8;
    VWB_DrawPic(vidbuf, backgroundPitch, i,TIMEY*8,L_NUM0PIC+(sec%10));
    ...
    {
        VWBL_DrawPic (30*8,TIMEY*8,C_TIMECODEPIC);
        fontnumber = 0;
...
{
    VW_FadeOut();
    VWBL_Bar(0,0,screenwidth,screenheight,0x82);                      // background

    CA_CacheGrChunk (PG13PIC);
    VWBL_DrawPic (216,110,PG13PIC);
    VW_UpdateScreen ();
...
void Write(int x,int y,char *string)
{
    ...
    for (i=0;i<strlen(string);i++)
        ...
        else
        {
            ...
            {
                case '!':
                    VWB_DrawPic(vidbuf, backgroundPitch, nx,ny,L_EXPOINTPIC);
                    nx+=8;
                    continue;

                case '\'':
                    VWB_DrawPic(vidbuf, backgroundPitch, nx,ny,L_APOSTROPHEPIC);
                    nx+=8;
                    continue;
                ...
                case 0x3a:     // ':'
                    ...
                    VWB_DrawPic(vidbuf, backgroundPitch, nx,ny,L_COLONPIC);
                    nx+=8;
                    continue;

                case '%':
                    VWB_DrawPic(vidbuf, backgroundPitch, nx,ny,L_PERCENTPIC);
                    break;

                default:
                    VWB_DrawPic(vidbuf, backgroundPitch, nx,ny,alpha[ch]);
            }
            nx+=16;
       }
...
void BJ_Breathe(void)
{
    ...
    if (GetTicks()-lastbreath>max)
    {
        which^=1;
        VWBL_DrawPic(0,16,pics[which]);
        VW_UpdateScreen();
        lastbreath=GetTicks();
        max=35;
    }
...
//
    ...
    VWB_DrawPic(vidbuf, backgroundPitch, 0,0,C_INTERMISSIONPIC);
    UNCACHEGRCHUNK(C_INTERMISSIONPIC);
#endif
    VWB_DrawPic(vidbuf, backgroundPitch, 0,16,L_GUYPIC);
    VL_UnlockSurface(backgroundSurface);
...
#endif
        byte *vidbuf = VL_LockSurface(backgroundSurface);
        VWB_DrawPic(vidbuf, backgroundPitch, i,10*8,L_NUM0PIC+(min/10));
        i+=2*8;
        VWB_DrawPic(vidbuf, backgroundPitch, i,10*8,L_NUM0PIC+(min%10));
        i+=2*8;
        Write(i/8,10,":");
        i+=1*8;
        VWB_DrawPic(vidbuf, backgroundPitch, i,10*8,L_NUM0PIC+(sec/10));
        i+=2*8;
        VWB_DrawPic(vidbuf, backgroundPitch, i,10*8,L_NUM0PIC+(sec%10));
...
boolean PreloadUpdate(unsigned current, unsigned total)
{
    ...
    VWB_Bar(vidbuf, backgroundPitch, WindowX + 5,WindowY + WindowH - 3,w,2,BLACK);
    w = ((long)w * current) / total;
    if (w)
    {
        VWB_Bar(vidbuf, backgroundPitch, WindowX + 5,WindowY + WindowH - 3,w,2,0x37); //SECONDCOLOR);
        VWB_Bar(vidbuf, backgroundPitch, WindowX + 5,WindowY + WindowH - 3,w-1,1,0x32);
    }
    VL_UnlockSurface(backgroundSurface);
    VW_UpdateScreen();
...
//      CA_CacheGrChunk (C_CODEPIC);
    ...
    byte *vidbuf = VL_LockSurface(backgroundSurface);
    VWB_DrawPic(vidbuf, backgroundPitch, 48,0,HIGHSCORESPIC);
    UNCACHEGRCHUNK (HIGHSCORESPIC);

    VWB_DrawPic(vidbuf, backgroundPitch, 4*8,68,C_NAMEPIC);
    VWB_DrawPic(vidbuf, backgroundPitch, 20*8,68,C_LEVELPIC);
    VWB_DrawPic(vidbuf, backgroundPitch, 28*8,68,C_SCOREPIC);
#ifndef UPLOAD
//      VWB_DrawPic(vidbuf, 35*8,68,C_CODEPIC);
#endif
    fontnumber=0;
    ...
    CA_CacheGrChunk (STARTFONT+1);
    VWB_DrawPic (vidbuf, backgroundPitch, 0,0,HIGHSCORESPIC);
...
#ifdef SPEAR
        if (s->completed == 21)
            VWB_DrawPic (vidbuf, PrintX+8,PrintY-1,C_WONSPEARPIC);
        else
#endif
        US_Print(buffer);
