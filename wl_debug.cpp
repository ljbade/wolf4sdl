...
void PictureGrabber (void)
{
    ...
    {                                               // 64000 bytes image data
        for(x=0;x<320;x++)
        {
            buffer[x]=vidbuf[i*screenpitch+x];
        }
...
void BasicOverhead (void)
{
    ...
    offx = screenwidth/2;
    offy = (screenheight-40-MAPSIZE*z)/2;
    ...
    for(x=0;x<MAPSIZE;x++)
        for(y=0;y<MAPSIZE;y++)
            VWB_Bar(vidbuf, backgroundPitch, x*z+offx, y*z+offy,z,z,(unsigned)actorat[x][y]);
    ...
    for(x=0;x<MAPSIZE;x++)
        for(y=0;y<MAPSIZE;y++)
        {
            ...
            VWB_Bar(vidbuf, backgroundPitch, x*z+offx, y*z+offy,z,z,color);
        }

    VWB_Bar(vidbuf, backgroundPitch, player->tilex*z+offx,player->tiley*z+offy,z,z,15); // player
...
//              addr = PM_GetPageAddress(i);
        ...
        if (addr)
        {
            if (i < PMSpriteStart)
            {
                ...
                // draw the wall
                //
                byte *vidbuf = VL_LockSurface(backgroundSurface) + 32*backgroundPitch;
                postx = 128;
                postwidth = 1;
                postsource = addr;
            ...
            else if (i < PMSoundStart)
            {
                ...
                // draw the sprite
                //
                GlobalSimpleScaleShape (VL_LockSurface(backgroundSurface)
                    + 32*backgroundPitch, 160, i-PMSpriteStart, 64);
                VL_UnlockSurface(backgroundSurface);
            }
            else if (i == ChunksInFile - 1)
            {
            ...
            else
            {
                ...
                for (j = 0;j < PageLengths[i];j += 32)
                {
                    ...
                    v2 /= 4;
                    if (v2 < 0)
                        VWB_Vlin(vidbuf, backgroundPitch, WindowY + WindowH - 32 + v2,
                            WindowY + WindowH - 32,
                            WindowX + 8 + (j / 32),BLACK);
                    else
                        VWB_Vlin(vidbuf, backgroundPitch, WindowY + WindowH - 32,
