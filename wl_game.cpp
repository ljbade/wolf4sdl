    ...
    for (y = 0;y < mapheight;y++)
        for (x = 0;x < mapwidth;x++)
        {
            ...
            if (tile < AREATILE)
            {
                ...
                actorat[x][y] = (objtype *) (unsigned int) tile;
            }
            else
            {
                // area floor
...
void DrawPlayBorderSides (void)
{
    ...
    VWB_Bar (vidbuf, backgroundPitch, 0, 0, xl - 1, screenheight - STATUSLINES, bordercol);
    VWB_Bar (vidbuf, backgroundPitch, xl + viewwidth + 1, 0, xl - 2, screenheight - STATUSLINES, bordercol);

    VWB_Bar (vidbuf, backgroundPitch, 0, 0, screenwidth, yl - 1, bordercol);
    VWB_Bar (vidbuf, backgroundPitch, 0, yl + viewheight + 1, screenwidth, yl - 1, bordercol);

    VWB_Vlin (vidbuf, backgroundPitch, yl - 1, yl + viewheight, xl - 1, 0);
    VWB_Vlin (vidbuf, backgroundPitch, yl - 1, yl + viewheight, xl + viewwidth, bordercol - 2);

    VWB_Hlin (vidbuf, backgroundPitch, xl - 1, xl + viewwidth, yl - 1, 0);
    VWB_Hlin (vidbuf, backgroundPitch, xl - 1, xl + viewwidth, yl + viewheight, bordercol - 2);
    VWB_Plot (vidbuf, backgroundPitch, xl - 1, yl + viewheight, bordercol - 3);
...
void DrawStatusBorder (byte color)
{
    ...
    VWB_Bar (vidbuf, backgroundPitch, 0, 0, screenwidth, screenheight-37, color);
    VWB_Bar (vidbuf, backgroundPitch, 0, screenheight-37, 8, 36, color);
    VWB_Bar (vidbuf, backgroundPitch, 0, screenheight-2, screenwidth, 2, color);
    VWB_Bar (vidbuf, backgroundPitch, screenwidth-8, screenheight-37, 8, 36, color);

    VWB_Bar (vidbuf, backgroundPitch, 9, screenheight-3, 97, 1, color - 1);
    VWB_Bar (vidbuf, backgroundPitch, 106, screenheight-3, 161, 1, color - 2);
    VWB_Bar (vidbuf, backgroundPitch, 267, screenheight-3, 44, 1, color - 3);
    VWB_Bar (vidbuf, backgroundPitch, screenwidth-9, screenheight-36, 1, 20, color - 2);
    VWB_Bar (vidbuf, backgroundPitch, screenwidth-9, screenheight-16, 1, 14, color - 3);
...
void DrawPlayBorder (void)
{
    ...
    VWB_Bar (vidbuf, backgroundPitch, 0, 0, screenwidth, screenheight - STATUSLINES, bordercol);

    xl = screenwidth / 2 - viewwidth / 2;
    yl = (screenheight - STATUSLINES - viewheight) / 2;
    VWB_Bar (vidbuf, backgroundPitch, xl, yl, viewwidth, viewheight, 0);
    ...
    int xoff = (screenwidth-320)/2;
    if(xoff)
    {
        VWB_Bar (vidbuf, backgroundPitch, 0, screenheight-STATUSLINES, xoff, STATUSLINES, bordercol);
        VWB_Bar (vidbuf, backgroundPitch, screenwidth-xoff, screenheight-STATUSLINES, xoff, STATUSLINES, bordercol);
    }
    VL_UnlockSurface(backgroundSurface);
...
//    for (i = 0;i < 3;i++)
    {
//        vbuf = (byte *)(0xa0000 + screenloc[i]);
        VWBL_DrawPic ((screenwidth-320)/2, screenheight - STATUSLINES, STATUSBARPIC);
        DrawPlayBorder ();
    }
...
void Died (void)
{
    ...
    InitFizzleFade();
    VWBL_Bar (viewscreenx, viewscreeny, viewwidth, viewheight, 4);
    VW_UpdateScreen();
